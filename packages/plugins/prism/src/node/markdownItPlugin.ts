...
const markdownItPlugin: MarkdownIt.PluginWithOptions<PrismPluginOptions> = (
  md: MarkdownIt,
  { name = 'Prism', vPreBlock = true, vPreInline = true, tabSize = 4 }: PrismPluginOptions = {},
  prismPlugins: PrismPluginType = {}
) => {
  md.renderer.rules.fence = (tokens, idx: number, options: MarkdownIt.Options) => {
    ...
    if (prismPlugins['copy-to-clipboard']) {
      ...
      attrList.push(
        `data-prismjs-copy-error="${(copy.options && copy.options.error) || 'Ctrl+C Copy'}"`
      );
      attrList.push(
        `data-prismjs-copy-success="${(copy.options && copy.options.success) || 'success'}"`
      );
    }
    ...
    let result = `<pre style="${styleList.join(' ')}" class=" ${classList.join(
      ' '
    )}" ${attrList.join(' ')}><code style="${styleList.join(' ')}">${code}</code></pre>`;
